...
public interface ProductStepRateRepository extends JpaRepository<ProductStepRate, Long> {
    ...
    @Query("SELECT r FROM ProductStepRate r WHERE r.product.id = :productId AND r.productionStep.id = :stepId AND r.effectiveDate <= :date ORDER BY r.effectiveDate DESC LIMIT 1")
