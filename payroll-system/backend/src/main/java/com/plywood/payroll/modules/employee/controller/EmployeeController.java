...
public class EmployeeController {
    ...
    public ResponseEntity<ApiResponse<List<EmployeeListResponse>>> getAll(
            @RequestParam(value = "search", required = false) String search) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST, employeeService.getAll(search)));
    }
    ...
    public ResponseEntity<ApiResponse<List<EmployeeBasicResponse>>> searchBasic(@RequestParam("search") String search) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST, employeeService.searchBasic(search)));
    }
    ...
    public ResponseEntity<ApiResponse<EmployeeResponse>> update(@PathVariable("id") Long id,
            @Valid @RequestBody EmployeeRequest request) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_UPDATE, employeeService.update(id, request)));
    }

    @PutMapping("/{id}/reset-password")
    @PreAuthorize("hasAuthority('SYSTEM_ADMIN')")
    @Operation(summary = "Đặt lại mật khẩu nhân viên")
    public ResponseEntity<ApiResponse<Void>> resetPassword(@PathVariable("id") Long id,
    ...
    public ResponseEntity<ApiResponse<List<EmployeeAuditLogResponse>>> getAuditLogs(@PathVariable("id") Long id) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST, employeeService.getAuditLogs(id)));
    }
    ...
    @Operation(summary = "Lấy danh sách NV thuộc tổ tại 1 ngày cụ thể (cho chấm công)")
    public ResponseEntity<ApiResponse<List<EmployeeResponse>>> getActiveAtDate(
            @RequestParam("teamId") Long teamId,
            @RequestParam("date") @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) java.time.LocalDate date) {
        return ResponseEntity.ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST,
                employeeService.getEmployeesActiveAtDate(teamId, date)));
    }

    @GetMapping("/history-period")
    @PreAuthorize("hasAuthority('EMPLOYEE_VIEW') or hasAuthority('SYSTEM_ADMIN')")
    @Operation(summary = "Lấy danh sách NV hoạt động trong 1 khoảng thời gian theo TeamProcess")
    public ResponseEntity<ApiResponse<List<EmployeeResponse>>> getHistoryPeriod(
        ...
        return ResponseEntity.ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST,
                employeeService.getEmployeesInHistoryPeriod(fromDate, toDate)));
    }

    @PostMapping("/import/preview")
    @PreAuthorize("hasAuthority('EMPLOYEE_EDIT') or hasAuthority('SYSTEM_ADMIN')")
    @Operation(summary = "Xem trước danh sách nhân viên từ file Excel")
    public ResponseEntity<ApiResponse<ImportResult<EmployeeRequest>>> importPreview(
    ...
    public ResponseEntity<ApiResponse<List<EmployeeResponse>>> importEmployees(@RequestParam("file") MultipartFile file)
            throws IOException {
        com.plywood.payroll.modules.excel.dto.response.ImportResult<EmployeeRequest> result = excelService
                .importEmployeesPreview(file);
        ...
        return ResponseEntity
                .ok(ApiResponse.success("Nhập dữ liệu thành công " + responses.size() + " nhân viên", responses));
    }

    @PatchMapping("/{id}/zk-device-id")
    @PreAuthorize("hasAuthority('EMPLOYEE_EDIT') or hasAuthority('SYSTEM_ADMIN')")
    @Operation(summary = "Cập nhật nhanh ID chấm công")
    public ResponseEntity<ApiResponse<Void>> updateZkDeviceId(@PathVariable("id") Long id,
            @RequestBody String zkDeviceId) {
    ...
    public ResponseEntity<ApiResponse<String>> updateAvatar(@PathVariable("id") Long id,
            @RequestParam("file") MultipartFile file) throws IOException {
    ...
    public ResponseEntity<ApiResponse<EmployeeNoteResponse>> addNote(@PathVariable("id") Long id,
            @Valid @RequestBody EmployeeNoteRequest request) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_CREATE, employeeService.addNote(id, request)));
    }
    ...
    public ResponseEntity<ApiResponse<List<SalaryProcessResponse>>> getSalaryHistory(@PathVariable("id") Long id) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST, employeeService.getSalaryHistory(id)));
    }
    ...
    public ResponseEntity<ApiResponse<List<TeamProcessResponse>>> getTeamHistory(@PathVariable("id") Long id) {
        return ResponseEntity
                .ok(ApiResponse.success(MessageConstants.SUCCESS_GET_LIST, employeeService.getTeamHistory(id)));
    }

    @PutMapping("/team-history/{id}")
    @PreAuthorize("hasAuthority('EMPLOYEE_EDIT') or hasAuthority('SYSTEM_ADMIN')")
    @Operation(summary = "Cập nhật bản ghi lịch sử tổ đội")
    public ResponseEntity<ApiResponse<Void>> updateTeamHistory(
            @PathVariable("id") Long id,
            @RequestBody com.plywood.payroll.modules.employee.dto.request.TeamHistoryUpdateRequest request,
            java.security.Principal principal) {
        String updatedBy = principal != null ? principal.getName() : "ADMIN";
        employeeService.updateTeamHistoryRecord(id, request.getTeamId(), request.getStartDate(), request.getEndDate(),
                request.getNote(), updatedBy);
        return ResponseEntity.ok(ApiResponse.success(MessageConstants.SUCCESS_UPDATE, null));
    }
}
