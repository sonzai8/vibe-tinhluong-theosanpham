...
/**
 * ProcessController - alias của ProductionStepController theo mô hình mới.
 * Endpoint /api/processes tương đương /api/production-steps.
 * Cả hai đều active để giữ backward compatibility.
 */
@RestController
@RequestMapping("/api/processes")
@RequiredArgsConstructor
@Tag(name = "Process", description = "Quản lý công đoạn sản xuất (Process = ProductionStep)")
public class ProcessController {
    ...
    @PostMapping
    @Operation(summary = "Tạo công đoạn mới")
    ...
    public ResponseEntity<ApiResponse<ProductionStepResponse>> update(
            @PathVariable("id") Long id,
            @Valid @RequestBody ProductionStepRequest request) {
